...
class UseFunctions {
	static async loadPlatformSettings(req: express.Request, res: express.Response, next: express.NextFunction) {
		...
		if ((req.cookies['last_news_update_read'] ? req.cookies['last_news_update_read'].trim() : undefined) !== (platformSettings.news.cookie_value ? platformSettings.news.cookie_value.trim() : undefined) && new Date() < platformSettings.news.expires) {
