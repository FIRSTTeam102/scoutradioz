...
router.get('/matchscoutingaudit', wrap(async (req, res) => {
	logger.addContext('funcName', 'matchscoutingaudit[get]');
	logger.info('ENTER');
	...
	for (let matchIdx = 0; matchIdx < matches.length; matchIdx++) {
		...
		for (let allianceIdx = 0; allianceIdx < allianceArray.length; allianceIdx++) {
			thisAlliance = allianceArray[allianceIdx];
			...
			if (matchScoutReports.length != 3) {
				...
				let frcTaxi = 0;
				for (robotIdx = 1; robotIdx < 4; robotIdx++)
	...
	//logger.debug('match=' + JSON.stringify(match));
	res.render('./reports/matchscoutingaudit', {
