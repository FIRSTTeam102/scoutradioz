...
router.get("/", async function(req, res) {
	...
	var collection = db.get("team~members");
...
router.post('/setscoutingpair', async function(req, res) {
	...
	*/
	...
	////// Update members in 'users' so that they're marked as "assigned" (and won't be available to choose afterwards)
		...
		for (var roleIdx = 0; roleIdx < roleArray.length; roleIdx++) {
			...
			for (var i = 0; i < 6; i++) {
				...
				if (teamScoutMap[thisTeamKey] == null) {
					...
					//logger.debug(thisFuncName + ">> Comparing: " + thisTeamKey + ", for role " + thisRole + " is " + thisPossibleAssignee);
					
					// Are they available?
					if (thisPossibleAssignee != null && scoutAvailableMap[thisPossibleAssignee] != null) {
						// Assign them!
						//logger.debug(thisFuncName + "** Assigning " + thisPossibleAssignee + " to " + thisTeamKey);
...
router.get("/swapmembers", async function(req, res) {
	...
	matchCol.find({ event_key: eventId, "alliances.red.score": -1 },{sort: {"time": 1}}, function(e, docs){
		...
		scoreDataCol.distinct("assigned_scorer", {"event_key": eventId, "time": { $gte: earliestTimestamp }}, function (e, docs) {
			var scorers = docs;
			logger.debug(thisFuncName + 'distinct assigned_scorers: ' + JSON.stringify(scorers));
