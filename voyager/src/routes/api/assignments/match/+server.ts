...
export const GET: RequestHandler = async ({ url }) => {
	const onlyAssigned = url.searchParams.get('onlyAssigned') !== null ? { assigned_scorer: { $ne: null } } : {};
	...
	let all: (MatchScouting & { team_name: string })[] = (await utilities.find('matchscouting', {
	...
	for (let match of all) match.team_name = teamNames[match.team_key];
