...
import type { LightUser, MatchScoutingLocal, ScouterRecordLocal, WithStringDbId } from '$lib/localDB';
import type { EventKey, MatchScouting, OrgKey, ScouterRecord, User } from 'scoutradioz-types';
import LZMA from './lzma';
import db from '$lib/localDB';

interface CompressedMatchScouting {
	_: 'sched', // Type of data
    org_key: string;
    event_key: string;
    year: number;
    teams: string;
...
function stringToScouterRecord(str: string): ScouterRecordLocal {
	let split = str.split(':');
	return {
		id: split[0],
		name: split[1]
...
function stringToTeam(str: string) {
	let split = str.split(':');
	return {
		team_key: split[0],
		team_name: split[1]
 ...
 */
export function encodeMatchScouting(data: MatchScoutingLocal[]): Promise<Uint8Array> {
	return new Promise((resolve, reject) => {
		...
		lzma.compress(str, 9, (result, err) => {
			...
			if (result) {
				...
				let uint8Array = new Uint8Array(result.map(num => num + 128));
				resolve(uint8Array);
			}
		}, (percent) => {});
	});
 ...
 */
export function decodeMatchScouting(data: Uint8Array) {
	return new Promise((resolve, reject) => {
		
		let lzmaData = Array.from(data).map(itm => itm - 128);
		
		lzma.decompress(lzmaData, async (result, err) => {
			// try-catch wrapper for the asserts
			try {
				if (err) return reject(err);
				let json: CompressedMatchScouting = JSON.parse(result);
				...
				for (let i = 0; i < teamStrings.length; i++) {
					...
					for (let j = 0; j < 12; j += 2) {
						...
						teams.push(stringToTeam(thisTeamStr));
					}
					...
					while (thisAssignedStrs.length > 0) {
						...
						// Someone assigned
							...
							let thisAssigned = stringToScouterRecord(scouterStrings[thisScouterIdx]);
					...
					while (thisActualStrs.length > 0) {
						...
						if (thisSubstr.startsWith('|')) {
							...
							let thisScouterIdx = parseInt(thisSubstr);
							let thisActual = stringToScouterRecord(scouterStrings[thisScouterIdx]);
...
function b64encode(x: number[]) { 
	return btoa(x.map(function(v){return String.fromCharCode(v + 128)}).join(''))
};
function b64decode(x: string) {
	// @ts-ignore
	return atob(x).split('').map(function(v) {return v.codePointAt(0) - 128});
};